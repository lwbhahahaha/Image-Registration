...
% Make colormap:
max_color = [47    85   94] ./ 255;
min_color = [51   213  255] ./ 255;

r_vec     = linspace(max_color(1), min_color(1), numel(data));
g_vec     = linspace(max_color(2), min_color(2), numel(data));
b_vec     = linspace(max_color(3), min_color(3), numel(data));

% Looks good:
% max_color = [  0    0    0] ./ 255;
% red_color = [51    213    255] ./ 255;
% min_color = [255  255  255] ./ 255;
% 
% 
% r_vec     = horzcat(linspace(max_color(1), red_color(1), numel(data) / 2), linspace(red_color(1), min_color(1), numel(data) / 2));
% g_vec     = horzcat(linspace(max_color(2), red_color(2), numel(data) / 2), linspace(red_color(2), min_color(2), numel(data) / 2));
% b_vec     = horzcat(linspace(max_color(3), red_color(3), numel(data) / 2), linspace(red_color(3), min_color(3), numel(data) / 2));

map = [r_vec; g_vec; b_vec]';

% SPECT colors:
% min_color    = [  0    0        0] ./ 255;
% yellow_color = [255  255        0] ./ 255;             
% red_color    = [255    0        0] ./ 255;
% purple_color = [126    0      227] ./ 255;
% max_color    = [255    255    255] ./ 255;
% n = 4;
% 
%             
%             
% r_vec       = [   linspace(max_color(1), yellow_color(1), numel(data)/n)...
%                linspace(yellow_color(1),    red_color(1), numel(data)/n)...
%                linspace(   red_color(1), purple_color(1), numel(data)/n)...
%                linspace(purple_color(1),    min_color(1), numel(data)/n)  ];
% 
% g_vec       = [   linspace(max_color(2), yellow_color(2), numel(data)/n)...
%                linspace(yellow_color(2),    red_color(2), numel(data)/n)...
%                linspace(   red_color(2), purple_color(2), numel(data)/n)...
%                linspace(purple_color(2),    min_color(2), numel(data)/n)  ];
% 
% b_vec       = [   linspace(max_color(3), yellow_color(3), numel(data)/n)...
%                linspace(yellow_color(3),    red_color(3), numel(data)/n)...
%                linspace(   red_color(3), purple_color(3), numel(data)/n)...
%                linspace(purple_color(3),    min_color(3), numel(data)/n)  ];
% r_vec      = flipdim(r_vec, 2);
% g_vec      = flipdim(g_vec, 2);
% b_vec      = flipdim(b_vec, 2);
% map = [r_vec; g_vec; b_vec]';


colormap(hsv);
% colormap(map);
% h=patch(X,Y,color_mat);
set(h,'linewidth',1,'edgecolor','none');
if ~isempty(param.lines)
  [X,Y]=gen_xy(zeros(param.lines),param);

  h=patch(X,Y,lin(zeros(param.lines)')');
    set(h,'linewidth',1,'facecolor','none','edgecolor','none');
end
