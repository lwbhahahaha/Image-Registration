function [ perf, perf_map ] = calc_organ_perfusion_filtered( v1_vol, v2_vol, organ_vol, input_conc, tissue_rho, vx_size, delta_time )
...
% Zero padding and 2*Median padding --> median should not be impacted by
% noisy perf measurements
lower_bound = 0;
upper_bound = 2*median(perf_map(organ_vol));
perf_map(perf_map < lower_bound) = lower_bound; % Removes all zero values prior to filtering by padding them to zero
perf_map(perf_map > upper_bound) = upper_bound; % Remove all non-real high flow values prior to filtering by padding them to 2*median

% Median filter perfusion data
perf_map = FastFilt3D(perf_map, logical(organ_vol), 10, @nanmedian);
