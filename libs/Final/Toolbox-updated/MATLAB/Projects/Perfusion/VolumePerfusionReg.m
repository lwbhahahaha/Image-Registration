function [ perf, perf_maps, mass, tags ] = VolumePerfusionReg( acq_path, mask_org_path, params )
...
% cur_TP.update(cur_step);
params_vx_size = get_vx_size([acq_path '/DICOM/']);  %%ISSUE HERE!!!
...
if length(aif_vec) > 2 %I.e. it is not manual
    params_v1_thresh_hu = aif_vec(v1_idx);
else
    params_v1_thresh_hu = aif_vec(1); %Otherwise, set the V1 thresh = to the 1st index 
end
...
elseif (any(strfind(params_perf_mode, 'MANUAL')) || any(strfind(params_perf_mode, 'RETRO')))
    ...
    if v1_idx == 1 %%%%% THIS IS WHERE THE ISSUE IS AT!
        params_v1_thresh_hu = aif_vec(v1_idx);
    elseif length(aif_vec) > 2
...
if length(aif_vec) > 2
    input_conc = mean([aif_vec(v1_idx) aif_vec(v2_idx)]); %If continuous, plug in
else
...
% 4.1: Load volumes
v1_vol = load_volume_idx(acq_path, v1_idx); % This remains correct
...
if ischar(params_xlsx_path) && (ischar(params_mask_reg_path) && length(params_mask_reg_labels) > 1)
    ...
    %regional_mask = loadMAT(params_mask_reg_path, 2); %%% Messed up, this
    %one works on Shants computer
