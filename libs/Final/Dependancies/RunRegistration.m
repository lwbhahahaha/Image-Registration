function RunRegistration(base_directory, reference_fname, tiff_relpath, MIP_relpath, output_relpath)
...
tiff_directory = strcat(base_directory, tiff_relpath);
...
if ~exist('RunNiftyReg.m', 'file')
    ...
    addpath(genpath('\\POLARIS\Data4_New\Travis\MATLAB\Projects\CardiacWorkflow'));
end
...
disp('RunRegistration: Converting from Tiff to NIfTI.');
ConvertTiff2CroppedNifti(tiff_directory, nii_directory, BB, -1);
...
if gpuDeviceCount > 0
    ...
    warning('RunRegistration:Suggestion',...
        ['No GPU device detected, defaulting to CPU version. Consider running'...
         ' on a machine with a CUDA enabled GPU such as Antares.']);
