function RunNiftyReg_mod(nii_directory, reg_directory, master_directory, reference_fname, f3d_opts)
...
%     end
    ...
    if ~exist(affine_file(2:end-1), 'file') % have to remove the " from either side
        Call_Niftyreg_mod('aladin', reference_fname, floating_fname, master_directory, nii_directory, reg_directory);
    else
    ...
    if ~exist(output_cpp(2:end-1), 'file') % have to remove the " from either side
        Call_Niftyreg_mod('f3d' , reference_fname, floating_fname, master_directory, nii_directory, reg_directory, f3d_opts);
    else
